...
int parseline(const char *cmdline, char **argv) 
{
    ...
    while (delim) {
	argv[argc++] = buf; // first set pointer as start of str
	*delim = '\0';  // then, cut the string at delim
	buf = delim + 1;
...
void do_bgfg(char **argv) 
{
    ...
    char *param = argv[1];
    int pid, jid, fg;
    fg = strcmp(argv[0], "fg") == 0;    // fg or not
...
void trim(char *str) {
    ...
    // find start of last consecutive white-spaces
    for (end = start + strlen(start) - 1; end > start; end--) {
        if (!isspace((unsigned char) end[0]) && end[0] != '\n')
            break;
    }

    *(end + 1) = 0;

    // shift from current "start" to begining of string
    if (start > str) {
